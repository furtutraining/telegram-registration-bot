...
# Safe CHANNEL_ID parsing — handles missing or empty string without crashing
_raw_channel = os.environ.get("CHANNEL_ID", "").strip()
try:
    CHANNEL_ID = int(_raw_channel) if _raw_channel else None
except ValueError:
    CHANNEL_ID = None
...
def get_db():
    """Return a new per-call connection (safe for Railway's threading model)."""
...
TIME_KB = {
    "en": [["🌅 Morning (1:00 AM – 2:30 AM)"], ["🌇 Afternoon (11:00 PM – 1:00 PM)"]],
    "am": [["🌅 ጠዋት (1:00 AM – 2:30 AM)"],    ["🌇 ከሰዓት (11:00 PM – 1:00 PM)"]],
    "om": [["🌅 Ganama (1:00 AM – 2:30 AM)"],        ["🌇 Waaree booda (11:00 PM – 1:00 PM)"]],
}
...
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with get_db() as _conn:
            total = _conn.execute("SELECT COUNT(*) FROM registrations").fetchone()[0]
            today = _conn.execute(
